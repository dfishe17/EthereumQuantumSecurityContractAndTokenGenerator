...
import { QuantumContractUI } from "./QuantumContractUI";
import { ERC20ContractUI } from "./ERC20ContractUI";
...
export default function App() {
  ...
  if (typeof window.ethereum === "undefined") {
    return (
      ...
      >
        <h1 style={{ textAlign: "center" }}>Quantum Portal Contract and Token Generator</h1>
        <a
    ...
    <Wrapper>
      ...
      >
        <VStack>
          ...
          >
            Quantum Portal Contract and Token Generator
          </h1>
      ...
      {/* QUANTUM CONTRACT CREATION */}
      {walletAddress && (
        <>
          <h2>Create New Quantum Contract</h2>
          <QuantumContractUI onDeploy={async (formData) => {
            try {
              const provider = new ethers.providers.Web3Provider(ethereum);
              const signer = provider.getSigner();
              if (!output || !output.contracts || !output.contracts["contract.sol"]) {
                throw new Error("Contract compilation output not available");
              }
              
              const contractOutput = output.contracts["contract.sol"]["QuantumResistantContract"];
              if (!contractOutput) {
                throw new Error("QuantumResistantContract not found in compilation output");
              }
              ...
              const contract = await factory.deploy(
                formData.name,
                formData.dilithiumKeyLength,
                formData.kyberKeyLength,
                formData.rainbowKeyLength
              );
          ...
          <h2>Create New ERC20 Token</h2>
          <ERC20ContractUI onDeploy={async (formData) => {
            try {
              const provider = new ethers.providers.Web3Provider(ethereum);
              const signer = provider.getSigner();
              if (!output || !output.contracts || !output.contracts["contracts/SimpleERC20.sol"]) {
                throw new Error("Contract compilation output not available");
              }
              
              const contractOutput = output.contracts["contracts/SimpleERC20.sol"]["SimpleERC20"];
              if (!contractOutput) {
                throw new Error("SimpleERC20 contract not found in compilation output");
              }
        ...
        >
          ...
          >
            <HStack style={{ gap: 8, alignItems: "center" }}>
              {generalState === "write" ? (
                <Badge style={{ backgroundColor: "var(--accent-warning-dimmer)", color: "var(--accent-warning-default)" }}>
                  <Edit2 size={12} style={{ marginRight: 4 }} />
                  {generalState}
                </Badge>
              ) : (
                <Badge style={{ backgroundColor: "var(--accent-primary-dimmer)", color: "var(--accent-primary-default)" }}>
                  <Eye size={12} style={{ marginRight: 4 }} />
                  {generalState}
                </Badge>
              )}
              <Badge style={{ opacity: 0.7 }}>{stateMutability}</Badge>
            </HStack>
            <HelpIndicator

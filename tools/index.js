...
async function compile() {
  ...
  for (const src of fs.readdirSync(rootDir)) {
    if (src.endsWith(".sol")) {
      sources[src] = {
        content: fs.readFileSync(path.join(rootDir, src), "utf8"),
      };
    }
  }

  // Read files from contracts directory
  if (fs.existsSync(contractsDir)) {
    for (const src of fs.readdirSync(contractsDir)) {
      if (src.endsWith(".sol")) {
        sources[`contracts/${src}`] = {
          content: fs.readFileSync(path.join(contractsDir, src), "utf8"),
        };
      }
    }
  }
